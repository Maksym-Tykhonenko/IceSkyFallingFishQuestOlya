...
type Props = NativeStackScreenProps<RootStackParamList, 'GameRelaxMode'>;
...
const STORAGE_RELAX_LAST = 'relax_last_result_v1';
...
function getWinner(counts: MatchCounts): CubeColor | 'draw' {
  const items: Array<[CubeColor, number]> = [
    ['blue', counts.blue],
    ['yellow', counts.yellow],
    ['red', counts.red],
  ];
  items.sort((a, b) => b[1] - a[1]);
  if (items[0][1] === items[1][1]) return 'draw';
  return items[0][0];
}
...
export default function GameRelaxModeScreen({ navigation }: Props) {
  ...
  const countsRef = useRef<MatchCounts>({ blue: 0, yellow: 0, red: 0 });
  ...
  const resolveMatchesFull = useCallback(() => {
    while (true) {
      ...
      countsRef.current.blue += removedCount.blue;
      countsRef.current.yellow += removedCount.yellow;
      countsRef.current.red += removedCount.red;
  ...
  const endGame = useCallback(async () => {
    ...
    await AsyncStorage.setItem(STORAGE_RELAX_LAST, JSON.stringify(payload));

    const prevRaw = await AsyncStorage.getItem(STORAGE_RELAX_TOTAL_TRIPLES);
    const prev: MatchCounts = prevRaw ? JSON.parse(prevRaw) : { blue: 0, yellow: 0, red: 0 };
    const next: MatchCounts = {
      blue: prev.blue + counts.blue,
      yellow: prev.yellow + counts.yellow,
      red: prev.red + counts.red,
    };
    await AsyncStorage.setItem(STORAGE_RELAX_TOTAL_TRIPLES, JSON.stringify(next));
  }, []);
  ...
  const onStart = () => {
    ...
    countsRef.current = { blue: 0, yellow: 0, red: 0 };
  ...
  const baseResultCardH = Math.min(safeH * (IS_TINY ? 0.34 : 0.30), 280);
  const resultCardH = IS_TINY ? Math.max(180, baseResultCardH - 30) : baseResultCardH;

  const showDim = finished;
  const resultMaxH = Math.min(fieldH * 0.94, safeH * (IS_TINY ? 0.70 : 0.62));
          ...
          <View style={{ width: fieldW, height: fieldH, alignItems: 'center', justifyContent: 'center' }}>
            ...
            <View style={styles.fieldOverlay} pointerEvents="none">
              ...
              {started && !finished && (
                <Image
                  ...
                  style={{ position: 'absolute', left: cubeTopLeftX, top: activeY, width: cubeSize, height: cubeSize }}
                  resizeMode="contain"
                />
              )}
            </View>
            ...
            {finished && (
              <View style={styles.resultOverlay} pointerEvents="box-none">
                <ScrollView
                  style={{ maxHeight: resultMaxH, width: '100%' }}
                  contentContainerStyle={{
                    alignItems: 'center',
                    justifyContent: 'center',
                    paddingVertical: (IS_TINY ? 4 : 10) * s,
                  }}
                    ...
                    >
                      <View style={[styles.resRow, { height: rowH }]}>
                        <View style={styles.leftRes}>
                          <Image source={CUBE_BLUE} style={{ width: iconSz, height: iconSz }} resizeMode="contain" />
                          <Text style={[styles.resLabel, { fontSize: 13.5 * s }]}>Blue</Text>
                        </View>
                        <Text style={[styles.resNum, { fontSize: 16 * s }]}>{counts.blue}</Text>
                      </View>

                      <View style={[styles.resRow, { height: rowH }]}>
                        <View style={styles.leftRes}>
                          <Image source={CUBE_YELLOW} style={{ width: iconSz, height: iconSz }} resizeMode="contain" />
                          <Text style={[styles.resLabel, { fontSize: 13.5 * s }]}>Yellow</Text>
                        </View>
                        <Text style={[styles.resNum, { fontSize: 16 * s }]}>{counts.yellow}</Text>
                      </View>

                      <View style={[styles.resRow, { height: rowH }]}>
                        <View style={styles.leftRes}>
                          <Image source={CUBE_RED} style={{ width: iconSz, height: iconSz }} resizeMode="contain" />
                          <Text style={[styles.resLabel, { fontSize: 13.5 * s }]}>Red</Text>
                        </View>
                        <Text style={[styles.resNum, { fontSize: 16 * s }]}>{counts.red}</Text>
                      </View>

                      <View style={{ flex: 1 }} />

                      <Text style={[styles.winnerText, { fontSize: 12.5 * s }]}>
                        Winner: {winner === 'draw' ? 'Draw' : winner.toUpperCase()}
                      </Text>
                    </View>
